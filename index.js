        ...
        if (interaction.customId == "ช่วยเหลือ") {
            await interaction.reply({
                embeds: [new Discord.EmbedBuilder()
                    .setColor("Green")
                    .setTitle("ราคายศ")
                    .setDescription(`เติมเงิน 180 บาท จะได้ยศ\n-> <@&${config.role180}>`)], ephemeral: true
            })
        }
    }
    if (interaction.type === 5) {
        if (interaction.customId === "topup") {
            ...
            tw(config.phone, codeInput).then(async re => {
                switch (re.amount) {
                    case 180:
                        if (interaction.member.roles.cache.has(config.role180)) {
                            ...
                            await interaction.guild.channels.cache.get(config.channellog).send({
                                embeds: [
                                    new Discord.EmbedBuilder()
                                        ...
                                        .addFields({
                                            name: `คุณได้รับยศ`,
                                            value: `• <@&${config.role180}>`
                                        })
                                        .setColor("Green")
                                ]
                            })
                        } else {
                            await interaction.member.roles.add(config.role180)
